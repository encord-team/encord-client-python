from datetime import datetime, timezone
from unittest.mock import Mock

import pytest
from deepdiff import DeepDiff

from encord.objects import Classification, LabelRowV2, Object
from encord.objects.frames import Range
from encord.objects.spaces.range_space import TextSpace
from tests.objects.data.all_types_ontology_structure import all_types_structure
from tests.objects.data.data_group.two_text import DATA_GROUP_TWO_TEXT_NO_LABELS, DATA_GROUP_WITH_TWO_TEXT_LABELS
from tests.objects.data.data_group.two_videos import (
    DATA_GROUP_METADATA,
)

text_obj_ontology_item = all_types_structure.get_child_by_hash("textFeatureNodeHash", Object)
text_classification = all_types_structure.get_child_by_hash("jPOcEsbw", Classification)


@pytest.fixture
def ontology():
    ontology_structure = Mock()
    ontology_structure.get_child_by_hash = all_types_structure.get_child_by_hash
    ontology = Mock(structure=ontology_structure)
    yield ontology


def test_text_space_can_add_object_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)
    # Check that no objects exist on space
    objects_on_text_space = text_space.get_object_instances()
    assert len(objects_on_text_space) == 0

    # After adding, check that object exists, and has correct properties
    created_by = "new_user"
    created_at = datetime(2020, 1, 1, tzinfo=timezone.utc)
    added_object_instance = text_space.add_object_instance(
        obj=text_obj_ontology_item,
        range=Range(start=10, end=100),
        created_by=created_by,
        created_at=created_at,
        manual_annotation=False,
    )
    objects_on_text_space = text_space.get_object_instances()
    assert len(objects_on_text_space) == 1

    first_object_instance = objects_on_text_space[0]
    annotations_on_first_object_instance = first_object_instance.get_annotations()
    assert first_object_instance.object_hash == added_object_instance.object_hash
    assert len(annotations_on_first_object_instance) == 1

    frames_on_annotations = [annotation.frame for annotation in annotations_on_first_object_instance]
    assert frames_on_annotations == [0]
    for annotation in annotations_on_first_object_instance:
        assert annotation.coordinates.range[0] == Range(start=10, end=100)
        assert annotation.created_by == created_by
        assert annotation.created_at == created_at
        assert annotation.manual_annotation is False

    # Check that spaces has correct properties
    assert len(text_space._objects_map.keys()) == 1
    assert added_object_instance.object_hash in text_space._objects_map


def test_text_space_can_remove_object_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)

    assert text_space is not None
    added_object_instance = text_space.add_object_instance(obj=text_obj_ontology_item, range=Range(start=10, end=100))
    objects_on_text_space = text_space.get_object_instances()

    # Check properties after adding object
    assert len(objects_on_text_space) == 1
    assert len(text_space._objects_map.keys()) == 1
    assert added_object_instance.object_hash in text_space._objects_map

    # Check properties after removing object
    text_space.remove_object_instance(added_object_instance.object_hash)
    objects_on_text_space = text_space.get_object_instances()
    assert len(objects_on_text_space) == 0
    assert len(text_space._objects_map.keys()) == 0


def test_text_space_move_object_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space_1 = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)
    object_on_text_space_1 = text_space_1.add_object_instance(
        obj=text_obj_ontology_item, range=Range(start=10, end=100)
    )
    objects_on_text_space_1 = text_space_1.get_object_instances()

    # Check properties of vision space 1 before moving object
    assert len(objects_on_text_space_1) == 1
    assert len(text_space_1._objects_map.keys()) == 1
    assert object_on_text_space_1.object_hash in text_space_1._objects_map

    # Check properties of vision space 2 before moving object
    text_space_2 = label_row.get_space_by_id("text-2-uuid", type_=TextSpace)
    objects_on_text_space_2 = text_space_2.get_object_instances()
    assert len(objects_on_text_space_2) == 0
    assert len(text_space_2._objects_map.keys()) == 0

    # Move object from space 1 to space 2
    text_space_2.move_object_instance_from_space(object_on_text_space_1)

    # Check properties of image space 1 after moving object
    objects_on_text_space_1 = text_space_1.get_object_instances()
    assert len(objects_on_text_space_1) == 0
    assert len(text_space_1._objects_map.keys()) == 0

    # Check properties of image space 2 after moving object
    objects_on_text_space_2 = text_space_2.get_object_instances()
    assert len(objects_on_text_space_2) == 1
    assert len(text_space_2._objects_map.keys()) == 1


def test_text_space_can_add_classification_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)
    # Check that no objects exist on space
    classifications_on_text_space = text_space.get_classification_instances()
    assert len(classifications_on_text_space) == 0

    # After adding, check that object exists, and has correct properties
    created_by = "new_user"
    created_at = datetime(2020, 1, 1, tzinfo=timezone.utc)
    added_classification_instance = text_space.add_classification_instance(
        classification=text_classification,
        created_by=created_by,
        created_at=created_at,
        manual_annotation=False,
    )
    classifications_on_text_space = text_space.get_classification_instances()
    assert len(classifications_on_text_space) == 1

    first_classification_instance = classifications_on_text_space[0]
    annotations_on_first_classification_instance = first_classification_instance.get_annotations()
    assert first_classification_instance.classification_hash == added_classification_instance.classification_hash
    assert len(annotations_on_first_classification_instance) == 1

    frames_on_annotations = [annotation.frame for annotation in annotations_on_first_classification_instance]
    assert frames_on_annotations == [0]
    for annotation in annotations_on_first_classification_instance:
        assert annotation.created_by == created_by
        assert annotation.created_at == created_at
        assert annotation.manual_annotation is False

    # Check that spaces has correct properties
    assert len(text_space._classifications_map.keys()) == 1
    assert added_classification_instance.classification_hash in text_space._classifications_map


def test_text_space_can_remove_classification_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)

    assert text_space is not None
    added_classification_instance = text_space.add_classification_instance(classification=text_classification)
    classifications_on_text_space = text_space.get_classification_instances()

    # Check properties after adding classification
    assert len(classifications_on_text_space) == 1
    assert len(text_space._classifications_map.keys()) == 1
    assert added_classification_instance.classification_hash in text_space._classifications_map

    # Check properties after removing classification
    text_space.remove_classification_instance(added_classification_instance.classification_hash)
    classifications_on_text_space = text_space.get_classification_instances()
    assert len(classifications_on_text_space) == 0
    assert len(text_space._classifications_map.keys()) == 0


def test_text_space_move_classification_instances(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_TWO_TEXT_NO_LABELS)

    text_space_1 = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)
    classification_on_text_space_1 = text_space_1.add_classification_instance(classification=text_classification)
    classifications_on_text_space_1 = text_space_1.get_classification_instances()

    # Check properties of vision space 1 before moving classification
    assert len(classifications_on_text_space_1) == 1
    assert len(text_space_1._classifications_map.keys()) == 1
    assert classification_on_text_space_1.classification_hash in text_space_1._classifications_map

    # Check properties of vision space 2 before moving classification
    text_space_2 = label_row.get_space_by_id("text-2-uuid", type_=TextSpace)
    classifications_on_image_space_2 = text_space_2.get_classification_instances()
    assert len(classifications_on_image_space_2) == 0
    assert len(text_space_2._classifications_map.keys()) == 0

    # Move classification from space 1 to space 2
    text_space_2.move_classification_instance_from_space(classification_on_text_space_1)

    # Check properties of vision space 1 after moving classification
    classifications_on_text_space_1 = text_space_1.get_classification_instances()
    assert len(classifications_on_text_space_1) == 0
    assert len(text_space_1._classifications_map.keys()) == 0

    # Check properties of vision space 2 after moving classification
    classifications_on_image_space_2 = text_space_2.get_classification_instances()
    assert len(classifications_on_image_space_2) == 1
    assert len(text_space_2._classifications_map.keys()) == 1


def test_read_and_export_labels(ontology):
    label_row = LabelRowV2(DATA_GROUP_METADATA, Mock(), ontology)
    label_row.from_labels_dict(DATA_GROUP_WITH_TWO_TEXT_LABELS)

    text_space_1 = label_row.get_space_by_id("text-1-uuid", type_=TextSpace)
    objects_on_text_space_1 = text_space_1.get_object_instances()
    assert len(objects_on_text_space_1) == 1

    text_object_instance = objects_on_text_space_1[0]
    assert text_object_instance.object_hash == "text1"
    assert text_object_instance.get_annotation_frames() == {0}
    assert text_object_instance.get_annotation().coordinates.range[0] == Range(start=10, end=50)

    classifications_on_text_space_1 = text_space_1.get_classification_instances()
    assert len(classifications_on_text_space_1) == 1
    classification_instance = classifications_on_text_space_1[0]
    assert classification_instance.classification_hash == "classification1"
    assert classification_instance.get_annotation_frames() == {0}

    output_dict = label_row.to_encord_dict()
    assert not DeepDiff(
        DATA_GROUP_WITH_TWO_TEXT_LABELS,
        output_dict,
        exclude_regex_paths=[r".*\['trackHash'\]"],
        ignore_order_func=lambda x: x.path().endswith("['objects']"),
    )
