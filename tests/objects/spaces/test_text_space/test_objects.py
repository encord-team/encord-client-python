...
def test_label_row_get_object_instances_on_space(ontology):
    ...
    text_space_1 = label_row.get_space(id="text-2-uuid", type_="text")
    ...
    text_space_1.put_object_instance(object_instance=object_instance_1, ranges=range_2)

    # Assert
    object_instances = label_row.get_object_instances()
    assert len(object_instances) == 2

    # For backwards compatibiity, all text objects are on frame=0
