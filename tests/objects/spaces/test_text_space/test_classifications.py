...
def test_label_row_get_classification_instances_on_text_space(ontology):
    ...
    text_space_2.put_classification_instance(classification_instance=classification_instance_1)

    # Assert
    all_classification_instances = label_row.get_classification_instances()
    assert len(all_classification_instances) == 2

    # For backwards compatibility, all text classifications are on frame=0
