...
from encord.orm.preset import Preset as OrmPreset, GetPresetParams, PresetFilter
...
class Preset:
    ...
    @property
    def filter_json(self) -> Optional[dict]:
        """
        ...
        Returns:
            Optional[str]: The preset description.
        """
        return self._preset_instance.filter_json
    ...
    @staticmethod
    def _get_presets(
        api_client: ApiClient, top_level_folder_uuid: UUID | None, preset_uuids
    ) -> "List[Preset]":
        params = GetPresetParams(topLevelFolderUuid=top_level_folder_uuid, uuids=preset_uuids)
        orm_item = api_client.get(
            "index/presets",
            params=params,
            result_type=GetPresetsResponse,
        )
        presets = [Preset(api_client, item) for item in orm_item.results]
        return presets
