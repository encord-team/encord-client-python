...
from encord.objects.spaces.types import FileInSceneInfo, PointCloudFileSpaceInfo, SpaceInfo
from encord.objects.types import (
    ObjectAnswer,
    ObjectAnswerForGeometric,
    ObjectAnswerForNonGeometric,
    SpaceSegmentationData,
)
...
class PointCloudFileSpace(RangeSpace):
    ...
    def _build_labels(self) -> dict[str, Any]:
        ...
        for obj in self.get_object_instances():
            ...
            # Convert ranges to point indices and encode as RLE
            points: set[int] = set()
            for range_obj in range_manager.get_ranges():
                for idx in range(range_obj.start, range_obj.end + 1):
                    points.add(idx)
            segmentation = points_to_rle_string(points)
        ...
        return {"objects": objects_list, "classifications": classifications_list}
    ...
    def _to_object_answers(self, existing_object_answers: Dict[str, ObjectAnswer]) -> dict[str, ObjectAnswer]:
        ...
        for obj in self.get_object_instances():
            ...
            if does_obj_exist:
                ...
                new_space_data: SpaceSegmentationData = {"range": [], "type": "frame", "segmentation": segmentation}
