...
def __add_frame_consensus_score(
    ...
    """
    Update consensus score when comparing base_frame's objects and classifications with consensus obtained from
    comparing_frames. Two objects' instances are considered the same object if their Jaccard similarity coefficient
    is greater or equal than the threshold; otherwise, they represent distinct objects.
    """
    ...
    # Score each base_frame's classification as true positive (TP) or false positive (FP)
    # when compared to consensus classifications
    for classification in base_frame["classifications"]:
        ...
        for class_answer in classifications_answers_list[0][classification_hash]["classifications"]:
            ...
            if isinstance(answers, str):  # text type attribute answer
                if attribute_feature_hash not in consensus_classifications[feature_hash].keys():
                    ...
                    score["FP"] += 1
            else:  # radio button or checklist attribute answer (list instance)
...
def __find_consensus_classifications(comparing_frames, feature_node_hash_set, classifications_answers_list):
    """
    Compare classifications generated by different agents across the same frame and return those where the majority of
    agents agreed on.
    """
    comparing_classifications = [frame["classifications"] for frame in comparing_frames]
    feature_hash_to_attribute_answers = dict()  # store attribute answers of each classification
    attributes_answers_count = dict()  # store occurrences of each attribute answer, and its value if its type is text
    # Calculate occurrences of each classification's attribute answer
    for index, classifications in enumerate(comparing_classifications):
        for classification in classifications:
            ...
            for class_answer in classifications_answers_list[index + 1][classification_hash]["classifications"]:
                answers = class_answer["answers"]
                attribute_feature_hash = class_answer["featureHash"]
                if isinstance(answers, str):  # encounter a text type attribute answer
                    # store (str_answer, occurrences) pairs for later check of the most repeated str_answer
                ...
                else:  # encounter a radio button or checklist attribute answer (list instance)
                    for answer in answers:
                        answer_hash = answer["featureHash"]
    ...
    amount_agents = len(comparing_frames)
    # Check if the majority of agents agreed on each classification's attribute answer and store the consensus
    for feature_hash, attribute_answers in feature_hash_to_attribute_answers.items():
        ...
        for attribute in attribute_answers:
            ...
            if isinstance(attribute_count, dict):  # found a text type attribute answer (str_answer, occurrences) pairs
            ...
            else:  # found a radio button or checklist attribute answer
