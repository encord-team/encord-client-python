...
from encord.beta.scene.internal.common import DEFAULT_CONVENTION as DEFAULT_CONVENTION
from encord.beta.scene.internal.common import Convention as Convention
from encord.beta.scene.internal.common import Direction as Direction
from encord.beta.scene.internal.common import DistortionModel as DistortionModel
from encord.beta.scene.internal.common import DivisionDistortionModel as DivisionDistortionModel
from encord.beta.scene.internal.common import FishEyeDistortionModel as FishEyeDistortionModel
from encord.beta.scene.internal.common import PinholeDistortionModel as PinholeDistortionModel
from encord.beta.scene.internal.common import PlumbBobDistortionModel as PlumbBobDistortionModel
from encord.beta.scene.internal.common import RadialDistortionModel as RadialDistortionModel
...
class CameraIntrinsicsSimple(BaseModel):
    ...
    model: Annotated[Optional[DistortionModel], Field(description="Distortion Model")] = None
    ...
    dfx: Annotated[Optional[float], Field(description="Distorted Focal length x")] = None
    dfy: Annotated[Optional[float], Field(description="Distorted Focal length y")] = None
    dox: Annotated[Optional[float], Field(description="Distorted Principal point offset x")] = None
    doy: Annotated[Optional[float], Field(description="Distorted Principal point offset y")] = None
    skew: Annotated[Optional[float], Field(description="Axis skew", validation_alias=AliasChoices("skew", "s"))] = None


class CameraIntrinsicsAdvanced(BaseModel):
    model_config = ConfigDict(extra="forbid")
    type: Literal["advanced"]
    model: Annotated[Optional[DistortionModel], Field(default=None, description="Distortion Model")]
    k: Annotated[
        Optional[list[float]],
        Field(default=None, description="Intrinsic Camera Matrix for Distorted Images", min_length=9, max_length=9),
    ]
    r: Annotated[
        Optional[list[float]], Field(default=None, description="Rectification Matrix", min_length=9, max_length=9)
    ]
    p: Annotated[
        Optional[list[float]], Field(default=None, description="Projection Camera Matrix", min_length=12, max_length=12)
    ]
    skew: Annotated[Optional[float], Field(description="Axis skew", validation_alias=AliasChoices("skew", "s"))] = None
...
class InputPose(RootModel):
    root: Annotated[
        Union[Annotated[InputCompositePose, Tag("pose")], Annotated[InputAffineTransform, Tag("affine")]],
        Field(
...
class InputCameraParams(BaseModel):
    ...
    extrinsics: Annotated[Optional[InputPose], Field(description="Camera exterinsics (deprecated)")] = None
