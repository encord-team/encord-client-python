...
class Scene:
    ...
    def load_point_cloud(
        ...
        url = stream_info.get_event_url(event_index)
        return PointCloud.from_url(url, timeout=timeout)
    ...
    async def load_point_cloud_async(
        ...
        stream_info = self._get_point_cloud_stream(stream_id)
        url = stream_info.get_event_url(event_index)
        return await PointCloud.from_url_async(url, timeout=timeout)
    ...
    async def load_all_point_clouds_async(
        ...
        async def load_with_limit(index: int) -> PointCloud:
            async with semaphore:
                url = stream_info.get_event_url(index)
                return await PointCloud.from_url_async(url, timeout=timeout)
    ...
    def load_point_cloud_in_world_coordinates(
        ...
        point_cloud = self.load_point_cloud(stream_id, event_index, timeout)
...
class PointCloudStreamInfo:
    ...
    def get_event_url(self, index: int) -> str:
        ...
        Returns:
            The signed URL for the point cloud file.
        ...
        return self._stream.events[index].uri
    ...
    def get_all_urls(self) -> List[str]:
        """Get all event URLs in order."""
        return [event.uri for event in self._stream.events]
...
class ImageStreamInfo:
    ...
    def get_event_url(self, index: int) -> str:
        ...
        Returns:
            The signed URL for the image file.
